...
from blog.api.serializers import ActivityPostSerializer, ActivityPostUpdateSerializer, ActivityPostCreateSerializer
...
def api_detail_blog_view(request, slug):
	...
	try:
		...
		activity_post = ActivityPost.objects.get(slug=slug)
...
@api_view(['GET', ])
@permission_classes((IsAuthenticated, ))
def api_detail_activity_view(request, slug):
...
@api_view(['PUT',])
@permission_classes((IsAuthenticated,))
def api_update_activity_view(request, slug):
	...
	if request.method == 'PUT':
		serializer = ActivityPostUpdateSerializer(activity_post, data=request.data, partial=True)
		data = {}
		if serializer.is_valid():
			serializer.save()
			data['response'] = UPDATE_SUCCESS
			data['pk'] = activity_post.pk
			data['title'] = activity_post.title
			data['start_time'] = activity_post.start_time
			data['end_time'] = activity_post.end_time
			data['slug'] = activity_post.slug

			data['username'] = activity_post.author.username
			return Response(data=data)
		return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
@api_view(['GET',])
@permission_classes((IsAuthenticated,))
def api_is_author_of_activitypost(request, slug):
	try:
		activity_post = ActivityPost.objects.get(slug=slug)
	except ActivityPost.DoesNotExist:
		return Response(status=status.HTTP_404_NOT_FOUND)
...
@api_view(['DELETE',])
@permission_classes((IsAuthenticated, ))
def api_delete_activity_view(request, slug):
	...
	if request.method == 'DELETE':
		operation = activity_post.delete()
		data = {}
		if operation:
			data['response'] = DELETE_SUCCESS
		return Response(data=data)
...
@api_view(['POST'])
@permission_classes((IsAuthenticated,))
def api_create_activity_view(request):

	if request.method == 'POST':

		data = request.data
		data['author'] = request.user.pk
		serializer = ActivityPostCreateSerializer(data=data)

		data = {}
		if serializer.is_valid():
			activity_post = serializer.save()
			data['response'] = CREATE_SUCCESS
			data['pk'] = activity_post.pk
			data['title'] = activity_post.title
			data['start_time'] = activity_post.start_time
